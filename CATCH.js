"use strict";
const catchVersion = "1.1.3";

// Singleton to prevent multiple instances
if (window.catchInstance) {
    if (window.catchInstance.cleanup) {
        window.catchInstance.cleanup();
    }
}

function catchAll() {
    // --- Shared Constants & Styles ---
    const PANEL_ID = "catch-panel";
    const FAB_ID = "catch-fab";
    const OVERLAY_CONTAINER_ID = "catch-overlay-container";
    ...
    const UI_STYLES = `
        ...
        #${PANEL_ID} .catch-tab:last-child { border-right: none; }
        #${PANEL_ID} .catch-tab:hover { background: #444; color: white; }
        #${PANEL_ID} .catch-tab.active {
            background: #005fcc;
            color: white;
        }
        ...
        .catch-list { list-style: none; margin: 0; padding: 0; }
        .catch-list li { margin-bottom: 4px; padding: 4px; background: #222; border-left: 3px solid #555; }
        .catch-name .catch-list li,
        .catch-image .catch-list li { color: #999;}
        .catch-name .catch-list li:first-child,
        .catch-image .catch-list li:first-child { border-left-color: #007bff; color: #fff;}
        .catch-tag { font-family: monospace; background: #333; padding: 1px 3px; border-radius: 2px; color: #fa0; }
        .catch-good { color: #99f170; font-weight: bold; }
        .catch-bad { color: #ff8888; font-weight: bold; }
        .catch-warn { color: #ffcc00; font-weight: bold; }

        .catch-indent-1 { margin-left: 0; border-left-color: #007bff !important; }
        .catch-indent-2 { margin-left: 10px; border-left-color: #28a745 !important; }
        .catch-indent-3 { margin-left: 20px; border-left-color: #ffc107 !important; }
        .catch-indent-4 { margin-left: 30px; border-left-color: #dc3545 !important; }
        .catch-indent-5 { margin-left: 40px; border-left-color: #17a2b8 !important; }
        .catch-indent-6 { margin-left: 50px; border-left-color: #6c757d !important; }

        /* Tools Module */
        .catch-tools-container { margin: 10px 0; }
        .catch-tools-btn {
            padding: 8px 16px;
            background: #005fcc;
            color: white;
        ...
        .catch-tools-btn:hover { background: #004a9e; }
        .catch-tools-desc {
            margin-top: 10px;
            font-size: 12px;
            color: #aaa;
        ...
        .catch-switch[aria-checked="true"] .catch-switch-track {
            background: #005fcc;
        }
        .catch-switch-thumb {
            width: 20px;
        ...
        .catch-switch[aria-checked="true"] .catch-switch-thumb {
            left: 22px;
        }
        .catch-switch:focus {
        ...
        .catch-contrast-section {
            margin-top: 16px;
            border-top: 1px solid #444;
            padding-top: 12px;
        }
        .catch-contrast-section h4 {
            margin: 0 0 10px;
            font-size: 13px;
            color: #ccc;
            font-weight: bold;
        }
        .catch-contrast-pickers {
            display: flex;
            gap: 16px;
            margin-bottom: 12px;
            align-items: flex-end;
        }
        .catch-contrast-picker-group {
            display: flex;
            flex-direction: column;
            gap: 4px;
        }
        .catch-contrast-picker-group label {
            font-size: 12px;
            color: #aaa;
        }
        .catch-contrast-picker-group input[type="color"] {
            width: 48px;
            height: 32px;
            border: 1px solid #555;
            border-radius: 4px;
        ...
        .catch-contrast-results {
            background: #1a1a1a;
            border-radius: 4px;
            padding: 8px 10px;
            font-size: 13px;
        }
        .catch-contrast-ratio {
            font-size: 20px;
            font-weight: bold;
            color: #fff;
            margin-bottom: 6px;
    ...
    function createUI() {
        ...
        overlayStyle.id = "catch-overlay-styles";
        ...
        fab.id = FAB_ID;
        fab.innerHTML = FAB_ICON + '<span>CATCH</span>';
        fab.setAttribute("role", "button");
        fab.setAttribute("aria-label", "CATCH open bookmarklet");
        ...
        panel.innerHTML = `
            <div id="${PANEL_ID}-header" role="tablist">
                <button class="catch-tab active" role="tab" aria-selected="true" tabindex="0" data-tab="overview">Overview</button>
                <button class="catch-tab" role="tab" aria-selected="false" tabindex="-1" data-tab="name">Name</button>
                <button class="catch-tab" role="tab" aria-selected="false" tabindex="-1" data-tab="image">Images</button>
                <button class="catch-tab" role="tab" aria-selected="false" tabindex="-1" data-tab="heading">Headings</button>
                <button class="catch-tab" role="tab" aria-selected="false" tabindex="-1" data-tab="tools">Tools</button>
            </div>
            <div id="${PANEL_ID}-content"></div>
            <div id="${PANEL_ID}-footer">
                <div class="catch-footer-links">by <a href="https://dirigointeractive.com" target="_blank">Dirigo Interactive</a></div>
                <button id="${PANEL_ID}-close">Close CATCH Panel</button>
                <button id="${PANEL_ID}-exit">Exit CATCH</button>
            </div>
        `;
        ...
        // Event Listeners
        const tabs = panel.querySelectorAll(".catch-tab");
        ...
        shadow.getElementById(`${PANEL_ID}-exit`).addEventListener("click", () => {
            if (window.catchInstance && window.catchInstance.cleanup) window.catchInstance.cleanup();
        });
    ...
    function togglePanel() {
        ...
        if (isVisible) {
            ...
            const firstTab = panel.querySelector(".catch-tab");
    ...
    function activateTab(tabName) {
        // UI Update
        panel.querySelectorAll(".catch-tab").forEach(btn => {
        ...
        contentArea.className = `catch-${tabName}`;
    ...
    const moduleModules = {
        overview: {
            activate: (container) => {
                container.innerHTML = `
                    <h3>CATCH - v${catchVersion}</h3>
                    <p>A hands-on companion to help you CATCH accessibility issues.</p>
                    <ul class="catch-list">
        ...
        name: {
            activate: (container) => {
                ...
                function handleFocus(e) {
                    ...
                    el.classList.add('catch-name-focus');
                    ...
                    let status = `<span class="catch-good">OK</span>`;
                    if (!info.name) status = `<span class="catch-bad">No Name</span>`;
                    ...
                    el.addEventListener('blur', () => el.classList.remove('catch-name-focus'), { once: true });
                }
                ...
                moduleCleanups.name = () => {
                    ...
                    document.querySelectorAll('.catch-name-focus').forEach(el => el.classList.remove('catch-name-focus'));
                };
            },
            deactivate: () => {
        ...
        image: {
            activate: (container) => {
                ...
                function handleHover(e) {
                    ...
                    // Status Color
                    let statusClass = "catch-good";
                    if (name === "Missing Alt" || name === "No Accessible Name") statusClass = "catch-bad";
                    if (name === "Decorative (empty alt)") statusClass = "catch-warn";
        ...
        heading: {
            activate: (container) => {
                ...
                headings.forEach(h => {
                    ...
                    div.className = "catch-heading-highlight";
                    // Apply styles
                    ...
                    label.className = "catch-heading-label";
                ...
                headings.forEach(h => {
                    ...
                    const li = document.createElement("li");
                    li.className = `catch-indent-${h.level}`;
                    // Set specific left border color for levels matches aHeading.js
                    // Note: catch-indent-X classes in STYLES handle border-color. 
                    // We need to ensure border-style is set in base class (checked: yes, 3px solid #555).

                    let warnings = "";
                    if (lastLevel > 0 && h.level > lastLevel + 1) warnings += ` <span class="catch-warn">[Skipped Level]</span>`;
                    if (!h.text) warnings += ` <span class="catch-bad">[Empty]</span>`;
                ...
                if (!hasH1 && headings.length > 0) {
                    const li = document.createElement("li");
                    li.innerHTML = `<span class="catch-bad">NO VISIBLE H1 FOUND</span>`;
                    ul.prepend(li);
                }
        ...
        tools: {
            activate: (container) => {
                container.innerHTML = `
                    ...
                    <ul class="catch-list">
                        <li>
                            <div role="switch" 
                                 ...
                                 id="structure-switch"
                                 tabindex="0">
                                <span class="catch-switch-track">
                                    <span class="catch-switch-thumb"></span>
                                </span>
                                <span id="structure-label">Page structure</span>
                            </div>
                            <div id="structure-desc" class="catch-tool-desc">
                    ...
                    <div class="catch-contrast-section">
                        <h4>Color contrast</h4>
                        <div class="catch-contrast-pickers">
                            <div class="catch-contrast-picker-group">
                                <label for="catch-fg-picker">Foreground</label>
                                <input type="color" id="catch-fg-picker" value="#000000">
                                <div class="catch-contrast-hex" id="catch-fg-hex">#000000</div>
                            </div>
                            <div class="catch-contrast-picker-group">
                                <label for="catch-bg-picker">Background</label>
                                <input type="color" id="catch-bg-picker" value="#ffffff">
                                <div class="catch-contrast-hex" id="catch-bg-hex">#ffffff</div>
                            </div>
                            <div class="catch-contrast-preview" id="catch-contrast-preview">
                                Aa
                            </div>
                        </div>
                        <div class="catch-contrast-results" id="catch-contrast-results">
                            <div class="catch-contrast-ratio" id="catch-contrast-ratio">21:1</div>
                            <div class="catch-contrast-row"><span>Normal Text <small>(&lt; 24px/19px bold)</small></span><span id="catch-cr-normal" class="catch-good">Pass (4.5:1)</span></div>
                            <div class="catch-contrast-row"><span>Large Text <small>(&ge; 24px/19px bold)</small></span><span id="catch-cr-large" class="catch-good">Pass (3:1)</span></div>
                            <div class="catch-contrast-row"><span>Non-text Elements:</span><span id="catch-cr-ui" class="catch-good">Pass (3:1)</span></div>
                        </div>
                    </div>
                `;
                ...
                function updateContrast() {
                    const fg = shadow.getElementById('catch-fg-picker').value;
                    const bg = shadow.getElementById('catch-bg-picker').value;
                    shadow.getElementById('catch-fg-hex').textContent = fg;
                    shadow.getElementById('catch-bg-hex').textContent = bg;

                    const preview = shadow.getElementById('catch-contrast-preview');
                    preview.style.color = fg;
                    preview.style.background = bg;

                    const ratio = contrastRatio(fg, bg);
                    const ratioStr = ratio.toFixed(2).replace(/\.?0+$/, '') + ':1';
                    shadow.getElementById('catch-contrast-ratio').textContent = ratioStr;

                    const passClass = 'catch-good';
                    const failClass = 'catch-bad';

                    const normalEl = shadow.getElementById('catch-cr-normal');
                    const largeEl = shadow.getElementById('catch-cr-large');
                    const uiEl = shadow.getElementById('catch-cr-ui');
                ...
                const fgPicker = shadow.getElementById('catch-fg-picker');
                const bgPicker = shadow.getElementById('catch-bg-picker');
    ...
    function activateStructure() {
        ...
        validElements.forEach(item => {
            item.element.classList.add('catch-structure-highlight');

            const labelDiv = document.createElement("div");
            labelDiv.className = "catch-structure-label";
    ...
    function deactivateStructure() {
        document.querySelectorAll('.catch-structure-highlight').forEach(el => el.classList.remove('catch-structure-highlight'));
    ...
    function deactivateTextSpacing() {
        ...
        const style = document.getElementById('catch-text-spacing');
        ...
        function removeFromShadows(root) {
            ...
            for (let i = 0; i < all.length; i++) {
                ...
                if (el.shadowRoot) {
                    try {
                        const shadowStyle = el.shadowRoot.getElementById('catch-text-spacing');
        ...
        iframes.forEach(iframe => {
            try {
                if (iframe.contentDocument) {
                    const iframeStyle = iframe.contentDocument.getElementById('catch-text-spacing');
    ...
    function cleanup() {
        ...
        if (document.getElementById("catch-overlay-styles")) document.getElementById("catch-overlay-styles").remove();
        if (host) host.remove();

        // Remove the script tag that loaded catch.js to allow re-initialization
        const script = document.getElementById("catch-script");
        if (script) script.remove();

        window.catchInstance = null;
    }
